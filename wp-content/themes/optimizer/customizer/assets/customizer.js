...
jQuery(document).ready(function() {
	...
	/*INNER TABS*/
		...
		//WHY PRO VIDEO CONTROLS
		jQuery('.whypro_vid i').on('click',function(event) {   jQuery("#wy_pro_vid, .whyprovid_close").show();  playerpromo.playVideo();  });
		jQuery('.whyprovid_close i').on('click',function(event) {   jQuery("#wy_pro_vid, .whyprovid_close").hide(); playerpromo.stopVideo();  });
...
var promodate = isPastDate('2016-10-01');

if(promodate == false){
	
	jQuery('.upgrade_btn').append('<i class="theme_notification">1</i>');
	jQuery('.upgrade_btn').after('<div class="chrismas_promo"><p><b>SEPTEMBER SALE:</b></br>Use coupon code "SEP20" to get 20% off on Optimizer PRO/Ultimate. Hurry Up! Offer Valid till 30th September, 2016.</p></div>');
...
function onYouTubeIframeAPIReady() {
    player = new YT.Player('business_video', {
        ...
        height: '',
        videoId: '1AdnB1piv1M',
        playerVars :{color: 'white', 'showinfo': 0, 'playlist':'1AdnB1piv1M'},
        events: {
            onReady: guide_initialize
        }
    });
    playerpromo = new YT.Player('wy_pro_vid', {
        width: '960',
        height: '580',
        videoId: 'FpmW2hGviqk',
        playerVars :{color: 'white', 'showinfo': 0, 'playlist':'FpmW2hGviqk'},
        events: {
            onReady: guide_initialize
        }
    });
